...
func second() {
	trimmed := strings.Trim(secondData, "\n")
	...
	for _, line := range lines {
		...
		policyString := strings.Trim(parts[0], " ")
		policyStringParts := strings.Split(policyString, " ")
		policyMinMax := strings.Split(policyStringParts[0], "-")
		policyLetter := strings.Trim(policyStringParts[1], " ")
		minInt, _ := strconv.ParseInt(policyMinMax[0], 10, 64)
		maxInt, _ := strconv.ParseInt(policyMinMax[1], 10, 64)
		...
		pass := password(strings.Trim(parts[1], " "))

		if validPassword2(policy, pass) {
			total++
		}
	}
...
func validPassword(policy passwordPolicy, pass password) bool {
	passLs := strings.Split(string(pass), "")

	count := 0
	for _, l := range passLs {
		if l == policy.letter {
			count++
		}
	}
...
func validPassword2(policy passwordPolicy, pass password) bool {
	passLs := strings.Split(string(pass), "")

	minCorrect := false
	maxCorrect := false

	for i, l := range passLs {
		if i == (policy.min-1) && l == policy.letter {
			minCorrect = true
		}

		if i == (policy.max-1) && l == policy.letter {
			maxCorrect = true
		}
	}

	if minCorrect && !maxCorrect || maxCorrect && !minCorrect {
		return true
	}
